...
var Model = class extends CustomType {
  constructor(websocket, connection_status, accounts, current_account, deposit_amount, withdraw_amount, new_account_id, new_account_holder, recent_transactions, last_error) {
    super();
...
var SetNewAccountId = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function parse_websocket_response(message) {
  let $ = contains_string(message, '"type":"event_notification"');
  if ($) {
    return new Ok("event_notification");
  } else {
    let $1 = contains_string(message, '"type":"account_snapshot"');
    if ($1) {
      return new Ok("account_snapshot");
    } else {
      let $2 = contains_string(message, '"type":"command_result"');
      if ($2) {
        return new Ok("command_result");
      } else {
        let $3 = contains_string(message, '"type":"connection_status"');
        if ($3) {
          return new Ok("connection_status");
        } else {
...
function parse_account_snapshot(message) {
  let _block;
  let $ = split2(message, '"account_id":"');
  if ($ instanceof Empty) {
    _block = new Error("account_id not found");
  } else {
    let $12 = $.tail;
    if ($12 instanceof Empty) {
      ...
      if ($22 instanceof Empty) {
        _block = new Error("Could not parse account_id");
      } else {
        let id = $22.head;
        _block = new Ok(id);
      }
  ...
  let account_id = _block;
  let _block$1;
  let $1 = split2(message, '"account_holder":"');
  if ($1 instanceof Empty) {
    _block$1 = new Error("account_holder not found");
  } else {
    let $22 = $1.tail;
    if ($22 instanceof Empty) {
      _block$1 = new Error("account_holder not found");
      ...
      let $3 = split2(rest, '"');
      if ($3 instanceof Empty) {
        _block$1 = new Error("Could not parse account_holder");
      } else {
        let holder = $3.head;
        _block$1 = new Ok(holder);
      }
    }
  }
  let account_holder = _block$1;
  let _block$2;
  let $2 = split2(message, '"balance":');
  if ($2 instanceof Empty) {
    _block$2 = new Error("balance not found");
  } else {
    let $3 = $2.tail;
    if ($3 instanceof Empty) {
      _block$2 = new Error("balance not found");
      ...
      let $4 = split2(rest, ",");
      if ($4 instanceof Empty) {
        let $5 = parse_float(rest);
        if ($5 instanceof Ok) {
          _block$2 = $5;
        } else {
          _block$2 = new Error("Could not parse balance");
        }
      } else {
        let balance_str = $4.head;
        let $5 = split2(balance_str, "}");
        if ($5 instanceof Empty) {
          _block$2 = new Error("Could not extract balance");
        } else {
          let balance_str$1 = $5.head;
          let $6 = parse_float(balance_str$1);
          if ($6 instanceof Ok) {
            _block$2 = $6;
          } else {
            _block$2 = new Error("Could not parse balance as float");
          }
        }
      }
    }
  }
  let balance = _block$2;
  if (balance instanceof Ok) {
    if (account_holder instanceof Ok) {
      if (account_id instanceof Ok) {
        let bal = balance[0];
        let holder = account_holder[0];
        let id = account_id[0];
        return new Ok([id, holder, bal]);
      } else {
        return account_id;
      }
    } else if (account_id instanceof Error) {
      return account_id;
    } else {
    ...
    return account_id;
  } else if (account_holder instanceof Error) {
    return account_holder;
  } else {
    return balance;
  }
}
function handle_event_notification(model, _) {
  let new_model = model;
  return [new_model, none()];
}
function handle_command_result(model, _) {
  let new_model = model;
  return [new_model, none()];
}
    ...
    toList([
      ...
      input(
        toList([
          ...
          value(model.new_account_id),
          on_input((var0) => {
            return new SetNewAccountId(var0);
          })
        ])
      ),
      input(
...
function handle_account_snapshot(model, message) {
  let $ = parse_account_snapshot(message);
  if ($ instanceof Ok) {
    let account_id = $[0][0];
    let account_holder = $[0][1];
    let balance = $[0][2];
    ...
    let new_model = new Model(
      ...
      model.new_account_holder,
      model.recent_transactions,
      new Some("Failed to parse account snapshot: " + error)
    );
    return [new_model, none()];
  }
}
function handle_websocket_message(model, message) {
  let debug_model = new Model(
    ...
    model.new_account_holder,
    model.recent_transactions,
    new Some("DEBUG: Received: " + message)
  );
  let $ = parse_websocket_response(message);
  if ($ instanceof Ok) {
    let parsed_message = $[0];
    if (parsed_message === "event_notification") {
      return handle_event_notification(debug_model, message);
    } else if (parsed_message === "account_snapshot") {
      return handle_account_snapshot(debug_model, message);
    } else if (parsed_message === "command_result") {
      return handle_command_result(debug_model, message);
    } else if (parsed_message === "connection_status") {
    ...
    let new_model = new Model(
      ...
      model.recent_transactions,
      new Some(
        "Failed to parse WebSocket message: " + error + " | Message: " + message
      )
    );
    return [new_model, none()];
  }
...
function update2(model, msg) {
  if (msg instanceof WebSocketMsg) {
    ...
    let new_model = new Model(
      ...
      model.new_account_holder,
      model.recent_transactions,
      new Some("DEBUG: Selected account: " + account_id)
    );
    let _block;
    let $ = model.websocket;
    if ($ instanceof Some) {
      let socket = $[0];
      _block = send_websocket_message_effect(
        socket,
        object2(
          toList([
            ["type", string3("subscribe_account")],
    ...
    let subscribe_effect = _block;
    return [new_model, subscribe_effect];
  ...
  } else if (msg instanceof SetNewAccountId) {
    let id = msg[0];
    return [
      new Model(
        model.websocket,
    ...
    if ($ instanceof Some) {
      let socket = $[0];
      let $1 = string_length(model.new_account_id) > 0;
      let $2 = string_length(model.new_account_holder) > 0;
      if ($2) {
        if ($1) {
          let command = object2(
            toList([
              ["type", string3("execute_command")],
              ["aggregate_id", string3(model.new_account_id)],
              [
                "command",
                object2(
                  toList([
                    ["type", string3("open_account")],
                    ["account_id", string3(model.new_account_id)],
          ...
          let snapshot_request = object2(
            toList([
              ["type", string3("request_snapshot")],
              ["account_id", string3(model.new_account_id)]
            ])
          );
          let snapshot_effect = send_websocket_message_effect(
            socket,
            snapshot_request
          );
          return [
            new_model,
            batch(toList([create_effect, snapshot_effect]))
          ];
        } else {
          let error = "Please fill in the account ID field (current: '" + model.new_account_id + "')";
          return [
            new Model(
              model.websocket,
              model.connection_status,
        ...
        let error = "Please fill in the account holder name field (current: '" + model.new_account_holder + "')";
        return [
          new Model(
            model.websocket,
            model.connection_status,
        ...
        let error = "Please fill in both account ID ('" + model.new_account_id + "') and account holder name ('" + model.new_account_holder + "')";
        return [
          new Model(
            model.websocket,
            model.connection_status,
    ...
    let new_transactions = prepend(
      transaction,
      take(model.recent_transactions, 19)
    );
    let new_model = new Model(
      model.websocket,
      model.connection_status,
